...
Public Class Prof_Camera_Form
    ...
    Private Sub Camera_Form_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.Focus()
        okBtn.Visible = False
        ...
        If cameraFeed.Image.Width = 1280 And cameraFeed.Image.Height = 720 Then
            ...
            Using imgResult As New Bitmap(CropRect.Width, CropRect.Height)
                Using grResult As Graphics = Graphics.FromImage(imgResult)
                    ...
                    imgResult.Save(Path.Combine(Application.StartupPath, "temp_prof_pic.jpg"))
                End Using
            End Using

            Using fs As New IO.FileStream(Path.Combine(Application.StartupPath, "temp_prof_pic.jpg"), IO.FileMode.Open, IO.FileAccess.Read)
                Prof_Form.usi_id_picture.Image = Image.FromStream(fs)
            End Using
    ...
    Private Sub Camera_Form_KeyPress(sender As Object, e As KeyPressEventArgs) Handles MyBase.KeyPress
